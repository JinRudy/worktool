...
class SettingsAdvanceActivity : AppCompatActivity() {
    ...
    private fun initView() {
        ...
        rl_local_port.setOnClickListener { showLocalPortDialog() }
        rl_local_callback.setOnClickListener { showLocalCallbackDialog() }
        rl_local_mode.setOnClickListener { showLocalModeDialog() }
    }
    ...
    private fun updateRobotQaUrl(callbackUrl: String) {
        try {
            ...
            val result = HttpHelper.postJson(Constant.getRobotUpdateUrl(), GsonUtils.toJson(json))
            if (result != null && JSONObject(result).getInt("code") == 200) {
    ...
    private fun showLocalPortDialog() {
        val builder = QMUIDialog.EditTextDialogBuilder(this)
        builder.setTitle("本地HTTP端口")
            .setPlaceholder("请输入本地HTTP服务器端口")
            .setDefaultText(Constant.localHttpPort.toString())
            ...
            .addAction(getString(R.string.add)) { dialog, index ->
                ...
                if (text != null) {
                    ...
                    if (port != null && port in 1024..65535) {
                        dialog.dismiss()
                        Constant.localHttpPort = port
                        ToastUtils.showLong("端口已更新: $port")
                    } else {
                        ToastUtils.showLong("端口范围: 1024-65535")
                    }
                } else {
                    ToastUtils.showLong("请勿为空！")
                }
    ...
    private fun showLocalCallbackDialog() {
        val builder = QMUIDialog.EditTextDialogBuilder(this)
        builder.setTitle("Bot服务器地址")
            .setPlaceholder("请输入Python服务器地址 (WebSocket)")
            ...
            .addAction(getString(R.string.add)) { dialog, index ->
                ...
                if (text != null) {
                    ...
                    if (url.isEmpty() || url.matches("https?://[^/]+.*".toRegex())) {
                        dialog.dismiss()
                        Constant.localCallbackUrl = url
                        ToastUtils.showLong(if (url.isEmpty()) "已清空" else "已更新")
                        // 地址变更后重连
    ...
    private fun showLocalModeDialog() {
        val checked = Constant.useLocalMode
        AlertDialog.Builder(this)
            .setTitle("本地模式")
            .setMessage("当前状态: ${if (checked) "已开启" else "未开启"}\n\n开启后将绕过云端API，直接通过WebSocket连接Python服务器")
            .setPositiveButton("开启") { _, _ ->
                Constant.useLocalMode = true
                ToastUtils.showLong("本地模式已开启")
            }
            .setNegativeButton("关闭") { _, _ ->
                Constant.useLocalMode = false
                ToastUtils.showLong("本地模式已关闭")
            }
