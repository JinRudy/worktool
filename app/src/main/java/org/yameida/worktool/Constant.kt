...
object Constant {
    ...
    val AVAILABLE_VERSION = arrayListOf("4.0.2", "4.0.6", "4.0.8", "4.0.10", "4.0.12", "4.0.16", "4.0.18", "4.0.19", "4.0.20", "4.1.0", "4.1.2", "4.1.3", "4.1.6", "4.1.7", "4.1.8", "4.1.9", "4.1.10", "5.0.9")
    ...
    var localHttpPort: Int
        get() = SPUtils.getInstance().getInt("localHttpPort", 8080)
        set(value) {
            SPUtils.getInstance().put("localHttpPort", value)
        }
    var localCallbackUrl: String
        get() = SPUtils.getInstance().getString("localCallbackUrl", "")
        set(value) {
            SPUtils.getInstance().put("localCallbackUrl", value)
        }
    // WebSocket 地址 = localCallbackUrl + "/ws/{robotId}"
    // 例: http://192.168.1.100:8000 → ws://192.168.1.100:8000/ws/abc123
    var useLocalMode: Boolean
        get() = SPUtils.getInstance().getBoolean("useLocalMode", false)
        set(value) {
            SPUtils.getInstance().put("useLocalMode", value)
        }
