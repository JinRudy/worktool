...
object WeworkOperationImpl {
    ...
    fun pushFile(
        ...
        if (fileUrl != null) {
            ...
            if (create && newFile.canWrite()) {
                if (HttpHelper.downloadFile(fileUrl, newFile)) {
    ...
    fun scanQrCode(message: WeworkMessageBean, fileUrl: String?, fileBase64: String?): Boolean {
        ...
        if (fileUrl != null) {
            ...
            val newFile = File(filePath)
            if (HttpHelper.downloadFile(fileUrl, newFile)) {
                ImageUtils.save(ImageUtils.getBitmap(newFile.readBytes().inputStream()), filePath, Bitmap.CompressFormat.JPEG, 100, true)
